...
class BengProxyHandler {
    ...
    function createBackendUri() {
        ...
        $accountPart = (isset($config["accountid"]) ? "/" . $config["accountid"] : "");
        ...
        foreach ($aliasData["map"] as $key => $value) {
            if (strpos($reqUri, $key) === 0) {
                ...
                if (strpos($key, "/.cm4all/") === 0) {
                    return $baseUri . "/1/" . $value;
                }

                $devicePart = ":";
                $designPart = $aliasData["design"]["desktop"];

                if ($isRawReq) {
                    $devicePart = ":raw";
                } else if (preg_match('/\/mobile\/index.php\//', $reqUri)) {
                    $devicePart = ":mobile";
                    if (isset($aliasData["design"]["mobile"])) {
                        $designPart = $aliasData["design"]["mobile"];
                    }
                } else if (preg_match('/\/desktop\/index.php\//', $reqUri)) {
                    $devicePart = ":desktop";
                }

                if (preg_match('/^sys\/([0-9A-Za-z_-]+)$/', $designPart, $matches)) {
                    $designPart = "sys-" . $matches[1];
                } else if (preg_match('/^oem\/([0-9A-Za-z_-]+)$/', $designPart, $matches)) {
                    $designPart = "oem-" . $matches[1];
                } else if (preg_match('/^user\/[0-9]+\/[0-9]+\/[0-9]+\/([0-9]+)$/', $designPart, $matches)) {
                    $designPart = "usr-" . $matches[1];
                } else if (preg_match('/^reseller\/([0-9]+)\/([0-9A-Za-z_-]+)$/', $designPart, $matches)) {
                    $designPart = "reseller/" . $matches[1] . "-" . $matches[2];
                } else if (preg_match('/^mobile\/([0-9A-Za-z_-]+)$/', $designPart, $matches)) {
                    $designPart = "mob-" . $matches[1];
                } else {
                    $designPart = "sys-" . $designPart;
                }

                if (preg_match('/^([0-9a-zA-Z]+)$/', $value, $matches)) {
                    return $baseUri . "/1/" . $matches[1] . ".umtl:" . $designPart . $devicePart . $this->requestUri;
